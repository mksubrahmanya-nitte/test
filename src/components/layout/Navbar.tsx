                    ...
                    {col.items.map((course, idx) => {
                      ...
                      if (course.hasSubMenu) {
                        return (
                          ...
                          >
                            <button
                              className="flex items-center justify-between w-full text-[12px] text-gray-700 hover:text-red-700 font-medium py-1.5 transition-colors text-left cursor-pointer"
                            >
                            ...
                            <div 
                              className={`absolute left-[95%] top-0 bg-white border border-gray-200 shadow-2xl rounded-lg p-4 w-72 flex flex-col gap-2 z-[100] animate-fade-in-down ml-1 ${
                                activeSubMenu === course.name ? 'flex' : 'hidden'
                              }`}
                      ...
                      return course.href === '#' ? (
                        <span 
                          key={idx} 
                          className="text-[12px] text-gray-400 cursor-default select-none font-medium py-1.5"
                          onMouseEnter={() => {
                            if (subMenuTimeoutRef.current) {
                              clearTimeout(subMenuTimeoutRef.current);
                              subMenuTimeoutRef.current = null;
